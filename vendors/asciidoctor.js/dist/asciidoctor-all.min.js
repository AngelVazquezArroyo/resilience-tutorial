...
"$read",function(a){var b=q;return function(){b=p.get("JAVASCRIPT_PLATFORM");if("node"["$==="](b))return require("fs").readFileSync(a,"utf8");if("java-nashorn"["$==="](b)){var c=Java.type("java.nio.file.Paths"),d=[];Java.type("java.nio.file.Files").readAllLines(c.get(a),Java.type("java.nio.charset.StandardCharsets").UTF_8).forEach(function(a){d.push(a)});return d.join("\n")}if("browser"["$==="](b)){var d="",f=-1;try{c=new XMLHttpRequest,c.open("GET",a,!1),c.addEventListener("load",function(){f=this.status;
